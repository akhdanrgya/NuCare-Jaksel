...
import {fetchKategoriBerita, KategoriBeritaType, fetchKategoriBeritaById} from "@/data/kategoriBerita";
...
const SelectBeritaKategori: React.FC<SelectKategoriProps> = ({ onChange, defaultValue }) => {
    const [selectedOption, setSelectedOption] = useState<string>(defaultValue.toString());
    const [isOptionSelected, setIsOptionSelected] = useState<boolean>(false);
    const [kategori, setKategori] = useState<KategoriBeritaType[]>([]);
    ...
    useEffect(() => {
        const fetchKategoriData = async () => {
            const dataKategori = await fetchKategoriBerita();
    ...
    useEffect(() => {
        const fetchKategoriByIdx = async () => {
            if (defaultValue) {
                const dataKategori = await fetchKategoriBeritaById(defaultValue);
                if (dataKategori) {
...
export default SelectBeritaKategori;
