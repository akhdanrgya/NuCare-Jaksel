...
import { fetchWakaf, insertWakaf, WakafType } from '@/data/wakaf';

const columns: { accessorKey: keyof WakafType; header: string }[] = [
...
export default function WakafPage() {
    ...
    const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        e.preventDefault();
        const newWakaf: WakafType = {
            id: data.length + 1,
            title: formData.title,
            amount: formData.amount,
        };

        const success = await insertWakaf(newWakaf);
        if (success) {
            setData(prev => [...prev, newWakaf]); // Update local state
    ...
    return (
        <section className={`${montserrat.variable} font-montserrat`}>
            ...
            <div className="container mx-auto px-4 sm:px-6 lg:px-8">
                {/* List */}
                <div className=''>
                    <Table columns={columns} data={data} source="wakaf" />
                </div>
                {/* Pagination */}
                <div className=''>
