...
const FormDonasi = ({ defaultValues }: { defaultValues?: DonasiType }) => {
  ...
  useEffect(() => {
    if (defaultValues) {
      ...
      setImageUrl(defaultValues.image || ""); // Set the image URL from default values if available
    }
  }, [defaultValues]);
  ...
  const uploadFile = async (file: File) => {
    ...
    if (uploadError) {
      ...
      setImageUrl("https://placehold.co/300x200"); // Default image URL if upload fails
      return null;
    }
