...
const FormDonasi = ({editing, defaultValues}: FormDonasiProps) => {
    ...
    useEffect(() => {
        const fetchSession = async () => {
            ...
            if (error) {
                setAlertMessage(`Error fetching session: ${error.message}`);
    ...
    const insertDonations = async (uploadedImageUrl: string | null) => {
        ...
        if (error) {
            setAlertMessage(`Gagal menyimpan data donasi : ${error.message}`);
            setAlertVisible(true)
        } else {
    ...
    const updateDonations = async (uploadedImageUrl: string | null) => {
        ...
        if (error) {
            setAlertMessage(`Gagal memperbarui data donasi : ${error.message}`);
            setAlertVisible(true)
        } else {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (editing && defaultValues?.id) {
                ...
                if (error) {
                    setAlertMessage(`Gagal memperbarui data berita: ${error.message}`)
                    setAlertVisible(true)
                } else {
                ...
                if (error) {
                    setAlertMessage(`Gagal menambahkan data berita: ${error.message}`);
                    setAlertVisible(true)
                } else {
            ...
            setAlertMessage(`Error handling submit: ${err}`)
            setAlertVisible(true)
        }
    };
